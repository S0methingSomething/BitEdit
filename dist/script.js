...
var AppMode;
(function (AppMode) {
    AppMode["Decrypt"] = "decrypt";
    AppMode["Encrypt"] = "encrypt";
})(AppMode || (AppMode = {}));
var ToastType;
(function (ToastType) {
    ToastType["Info"] = "info";
...
// --- App Initialization ---
const ui = {};
const DEFAULT_SOURCE_INFO = Object.freeze({
...
let currentSourceInfo = { ...DEFAULT_SOURCE_INFO };
...
function assertElementExists(element, id) {
    if (!element) {
...
function xorAndBase64Encode(text, key) {
    let xorResult = "";
...
function decryptInt(base64String) {
    try {
        ...
        if (bytes.length !== CONSTANTS.USER_SERIALIZED_INT32_TOTAL_LENGTH)
            return null;
        const prefix = bytes.slice(0, CONSTANTS.USER_SERIALIZED_INT32_PREFIX.length);
        const suffix = bytes[bytes.length - 1];
        if (!arraysEqual(prefix, CONSTANTS.USER_SERIALIZED_INT32_PREFIX) || suffix !== CONSTANTS.USER_SERIALIZED_INT32_SUFFIX[0])
            return null;
...
function compareVersions(v1, v2) {
    const normalize = (v) => v.trim().replace(/^v/, '').split('.').map(p => parseInt(p, 10)).filter(p => !isNaN(p));
...
function showStandardModal(modalElement) {
    modalElement.classList.remove('hidden');
    setTimeout(() => modalElement.classList.add('visible'), 10);
}
function hideStandardModal(modalElement) {
...
function processEncryptedContent(content, key) {
...
function serializeAndEncryptJson(jsonObject, key) {
    const obfuscatedKey = getObfuscatedKey(key);
    let fileContent = "";
...
async function handleManualFileProcessing() {
    clearError();
...
function cacheUIElements() {
    ...
    for (const id of ids) {
        ...
        ui[camelCaseId] = assertElementExists(document.getElementById(id), id);
    }
}
